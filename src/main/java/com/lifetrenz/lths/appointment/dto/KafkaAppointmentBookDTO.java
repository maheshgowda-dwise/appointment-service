package com.lifetrenz.lths.appointment.dto;

import java.io.Serializable;

import com.fasterxml.jackson.annotation.JsonIgnoreProperties;

import lombok.AllArgsConstructor;
import lombok.Data;
import lombok.NoArgsConstructor;

@Data
@AllArgsConstructor
@NoArgsConstructor
@JsonIgnoreProperties(ignoreUnknown = true)
public class KafkaAppointmentBookDTO implements Serializable {

	private static final long serialVersionUID = 1353321042158899130L;

	private String mrn;

	private RelayPatientRegistrationDTO kafkaEMRPatient;

	private LTCSBookAppointment bookAppointmentDTO;

	private CustomerTransactionAttributeDTO customerTransactionAttributeDTO;

	// *********** this DTO used as kafka make sure handled properly ******* //

}
